...
#include "mprmpr/rpc/request_tracker.h"
...
class ResultTracker : public base::RefCountedThreadSafe<ResultTracker> {
 public:
  ...
  explicit ResultTracker(std::shared_ptr<mprmpr::MemTracker> mem_tracker);
  ~ResultTracker();
 ...
 private:
  ...
  struct ClientState {
    ...
    void GCCompletionRecords(const std::shared_ptr<mprmpr::MemTracker>& mem_tracker,
