...
#include "mprmpr/util/trace_metrics.h"
...
#define ADOPT_TRACE(t) mprmpr::ScopedAdoptTrace _adopt_trace(t);
...
#define TRACE(format, substitutions...) \
  do { \
    mprmpr::Trace* _trace = Trace::CurrentTrace(); \
    if (_trace) { \
...
#define TRACE_COUNTER_SCOPE_LATENCY_US(counter_name) \
  ::mprmpr::ScopedTraceLatencyCounter _scoped_latency(counter_name)
...
class Trace : public base::RefCountedThreadSafe<Trace> {
 public:
  ...
  // 'call mprmpr::Trace::DumpCurrentTrace();'.
