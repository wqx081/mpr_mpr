...
METRIC_DEFINE_gauge_uint64(server, spinlock_contention_time,
    "Spinlock Contention Time", mprmpr::MetricUnit::kMicroseconds,
    ...
    mprmpr::EXPOSE_AS_COUNTER);
METRIC_DEFINE_gauge_uint64(server, tcmalloc_contention_time,
    "TCMalloc Contention Time", mprmpr::MetricUnit::kMicroseconds,
...
// mprmpr namespace so we don't need to qualify everything.
namespace gutil {
void SubmitSpinLockProfileData(const void *contendedlock, int64 wait_cycles) {
  mprmpr::SubmitSpinLockProfileData(contendedlock, wait_cycles);
}
} // namespace gutil
...
void SubmitSpinLockProfileData(const void* contendedlock, int64 wait_cycles) {
  ...
  auto t = mprmpr::Trace::CurrentTrace();
  if (t) {
    t->metrics()->IncrementTcmallocContentionCycles(wait_cycles);
  }
#endif // !defined(__APPLE__)
  base::subtle::NoBarrier_AtomicIncrement(&mprmpr::g_tcmalloc_contention.val, wait_cycles);
}
} // namespace base
