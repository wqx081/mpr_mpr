#include "mprmpr/tests/rpc/rpc-test-base.h"
...
static void AssertShutdown(mprmpr::Thread* thread, const Status* status) {
...
TEST_F(MultiThreadedRpcTest, TestShutdownDuringService) {
  ...
  scoped_refptr<mprmpr::Thread> threads[kNumThreads];
  Status statuses[kNumThreads];
  for (int i = 0; i < kNumThreads; i++) {
    ASSERT_OK(mprmpr::Thread::Create("test", strings::Substitute("t$0", i),
...
TEST_F(MultiThreadedRpcTest, TestShutdownClientWhileCallsPending) {
  ...
  scoped_refptr<mprmpr::Thread> thread;
  Status status;
  ASSERT_OK(mprmpr::Thread::Create("test", "test",
...
TEST_F(MultiThreadedRpcTest, TestBlowOutServiceQueue) {
  ...
  scoped_refptr<mprmpr::Thread> threads[3];
  Status status[3];
...
TEST_F(MultiThreadedRpcTest, TestShutdownWithIncomingConnections) {
  ...
  vector<scoped_refptr<mprmpr::Thread> > threads;
  for (int i = 0; i < 8; i++) {
    scoped_refptr<mprmpr::Thread> new_thread;
    CHECK_OK(mprmpr::Thread::Create("test", strings::Substitute("t$0", i),
  ...
  for (scoped_refptr<mprmpr::Thread>& t : threads) {
